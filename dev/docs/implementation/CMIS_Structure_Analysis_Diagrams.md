...
**Note**: Greenfield = 시장 전체 분석, focal_actor_context_id 불필요
...
sequenceDiagram
    ...
    SA->>WE: ingest_focal_actor_context()
    ...
    SA->>PCS: PRJ-* 저장
    PCS-->>SA: focal_actor_context_id 반환

    Note over SA: PH00 완료 → PH01-PH14 진행
    SA-->>User: focal_actor_context_id + 분석 시작
```
