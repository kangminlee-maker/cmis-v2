...
- **구현 요소**
  ...
  - FocalActorContext: capability_traits 기반 execution fit
...
- **특징**
  ...
  - FocalActorContext(우리 회사/프로젝트) overlay 지원
...
- FocalActorContext 버전업(ContextLearner)
...
- **Greenfield**: “나(FocalActorContext)” 없이 시장을 중립적으로 분석
  - FocalActorContext 없음, focal_actor 없음
  - 단, **최소 제약(자본/기간/팀 규모)** 는 입력 가능
- **Brownfield**: “나(FocalActorContext)” 관점에서 분석
  - FocalActorContext 있음, focal_actor 있음
  - 제약/자산/기준선(baseline_state) 반영

> Greenfield ≠ 신규, Brownfield ≠ 확장
> 핵심은 “나(FocalActorContext)의 존재 여부”입니다.
...
- EvidenceStore, ValueStore, Graph Snapshots(R/P/V/D), FocalActorContext, Strategy/Scenario 산출물
...
LearningEngine.update_from_outcomes
        ↓
Pattern benchmarks / Metric belief / FocalActorContext versioning
        ↓
(Understand로 루프)
```
...
- [ ] Greenfield/Brownfield가 “FocalActorContext 유무”로 일관되게 동작하는가?
