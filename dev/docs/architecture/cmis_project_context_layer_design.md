...
# CMIS: Project Context Layer (FocalActorContext)
사용자/조직 상황을 반영하는 Brownfield-aware 설계

> 주: 본 문서는 UMIS v9 시절 작성된 원문을 기반으로 하며, 현재 CMIS v3.6.0 기준으로 용어/경로를 최소 정렬했습니다.
> 본문에 남아있는 “v9/UMIS” 표기는 역사적 맥락으로 읽어주세요.

---

## 0. 문서 목적

이 문서는 CMIS 아키텍처(Reality / Pattern / Value / Decision Graph, Plane 분리, canonical workflows)를 유지하면서,
...
- `cmis.yaml`에서
  ...
  이 명확하게 정의되어 있습니다.
- `CMIS_Architecture_Blueprint_v3.6.0_km.md`에서
  - Interaction / Role / Substrate / Cognition 4 Plane과
  - Evidence/World/Pattern/Value/Strategy/Learning/Policy Engine 구조가 정리되어 있습니다.
- `config/workflows.yaml`의 `canonical_workflows`에서
  - `structure_analysis` / `opportunity_discovery` / `strategy_design` 워크플로우 실행 단위가 정의됩니다.
...
Substrate Plane의 `stores` 아래에 `focal_actor_context_store`를 추가하는 형태로 설계합니다.
...
- `snapshot(as_of, scope, focal_actor_context_id=None)`
  - focal_actor_context_id가 없으면:
    기존처럼 “시장 전체 Reality” 서브그래프 반환
  - focal_actor_context_id가 있으면:
...
PatternEngine는 호출 시 `focal_actor_context_id`를 선택적으로 인자로 받아,
...
2. 프로젝트 레벨 Metric
   ...
   - context: focal_actor_context_id + domain_id / region / year …

MetricSpec에 `context_requirements.level = market / project`와 같이 표현하면,

- ValueEngine은 `focal_actor_context_id`가 필요한 Metric과 그렇지 않은 Metric을 구분해 처리할 수 있습니다.
- 예:
  - `MET-SAM`: market level (focal_actor_context_id 없어도 계산)
  - `MET-SOM_for_project`: project level (focal_actor_context_id 없으면 fail)
...
- focal_actor_context_id (optional)

이렇게 하면,

- “Adult Language KR 시장 전체에서의 전략 목표” (focal_actor_context_id 없음)와
- “특정 회사의 Adult Language KR 진입 목표” (focal_actor_context_id 있음)를
  명확히 구분할 수 있습니다.

StrategyEngine은

- `search_strategies(goal_id, constraints, focal_actor_context_id=None)` 형태로 API를 확장해,
- focal_actor_context_id가 주어졌을 때:
...
2. `ingest_focal_actor_context` 유틸리티
...
- 입력: graph_slice_ref, focal_actor_context_id(optional)
...
1. Metric Request에 focal_actor_context_id 포함 가능
...
- 입력: goal_id, constraints, focal_actor_context_id(optional)
- 동작:
  1. Goal에 focal_actor_context_id가 연결되어 있지 않더라도,
     - 입력 인자로 받은 focal_actor_context_id를 통해
...
`config/workflows.yaml`의 `canonical_workflows`는 workflow 실행 단위를 정의합니다.
여기에 PH00(프로젝트 컨텍스트 준비)을 선행 단계로 두어, 구조 분석이 항상 “프로젝트 컨텍스트” 위에서 시작되도록 확장할 수 있습니다.

PH00의 목표:

- 사용자/조직의 현재 상태/자산/제약/선호를 구조화하여
  `focal_actor_context_store`와 R-Graph에 반영.
...
- PH01 이후 단계에서 사용할 reference (focal_actor_context_id)
...
1. PH00에서 생성한 focal_actor_context_id를 기반으로
   - WorldEngine.snapshot(as_of, scope, focal_actor_context_id) 호출
2. R-Graph에서 focal_actor와 주변 구조를 강조한 맵을 생성
3. PatternEngine.match_patterns(graph_slice_ref, focal_actor_context_id)
...
1. WorldEngine.snapshot(..., focal_actor_context_id)
2. PatternEngine.discover_gaps(graph_slice_ref, focal_actor_context_id)
...
`strategy_design`에 focal_actor_context_id를 추가하면,
같은 Goal이라도 Project Context에 따라 전략 후보가 달라집니다.

입력:

- goal_id (focal_actor_context_id와 연결)
- 또는 goal_id + focal_actor_context_id

단계:

1. StrategyEngine.search_strategies(goal_id, constraints, focal_actor_context_id)
   - execution_fit_score / constraints_profile를 활용해 전략 후보 생성/필터링
2. ValueEngine.simulate_scenario(scenario_id, policy_ref, focal_actor_context_id)
...
`focal_actor_context_id`만 달라졌을 뿐인데,
...
- Substrate Plane에 `focal_actor_context_store`만 추가.
- R-Graph에는 “클라이언트 회사” Actor와 관련 노드가 추가되지만,
  이는 원래 설계에서도 자연스러운 확장입니다.
- Decision Graph의 Goal/Strategy에 `focal_actor_context_id` 필드를 추가하는 수준의 변화.
...
새로운 거대 엔진을 만들지 않고,
기존 엔진의 API 입력에 `focal_actor_context_id`라는 하나의 축을 추가하는 형태입니다.
...
- CMIS는 “시장 OS”이자 “의사결정 OS”로 동작해야 하며,
  실제 사용 시에는 항상 특정 조직/프로젝트의 관점이 존재한다고 가정했습니다.
- 기존 R/P/V/D 분해 구조와 Plane 분리 설계는
  이미 잘 정립되어 있다고 보고,
  이를 변경하기보다는 “위에 얇은 컨텍스트 레이어를 얹는 방식”이 더 안전하다고 전제했습니다.

### 9.2 근거

- `cmis.yaml`의 `planes`/`substrate_plane`/`cognition_plane` 구조를 보면,
  시장/패턴/값/결정에 대한 스키마와 엔진은 잘 정의되어 있지만,
  사용자/조직의 현재 상황을 저장하는 Store는 존재하지 않습니다.
- `dev/docs/architecture/CMIS_Architecture_Blueprint_v3.6.0_km.md`에서
  CMIS의 대표 질문은 “한국 성인 어학교육 시장 구조/기회/진입전략”이지만,
  이 역시 특정 조직 관점보다는 시장 관점을 기준으로 설명됩니다.
- `config/workflows.yaml`의 `canonical_workflows`도
  시장 구조 분석/기회 발굴/전략 설계를 실행 단위로 제공하지만,
