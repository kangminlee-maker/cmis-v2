...
- Execution Fit 평가 (Reality + FocalActorContext 기반)
- Portfolio 평가 (여러 Strategy 조합)
- Scenario 시뮬레이션

**설계 철학**:
- **Model-first**: 전략은 Pattern 조합의 구조적 표현
- **Evidence-first**: 실행 가능성은 Reality/FocalActorContext 기반
...
- "실행 가능한가"는 Reality/FocalActorContext가 판단
...
- FocalActorContext 없으면 시장 평균 가정
...
**capability_match**:
- Strategy.required_capabilities vs FocalActorContext.assets_profile.capability_traits

**budget_sufficiency**:
- 예상 비용 (Pattern benchmark) vs FocalActorContext.constraints.max_budget

**time_sufficiency**:
- 예상 기간 vs Goal.target_horizon

**team_sufficiency**:
- 필요 인력 vs FocalActorContext.assets_profile.organizational_assets.team_size
...
- [x] **Evidence-first**: Execution Fit은 Reality/FocalActorContext 기반
...
- [x] **FocalActorContext**: 자원/제약 → Feasibility
