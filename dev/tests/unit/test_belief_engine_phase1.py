...
def _isolate_storage_root(tmp_path, monkeypatch) -> None:
    """BeliefEngine 관련 테스트의 value_store를 테스트별로 격리합니다."""
...
def test_monte_carlo_basic(tmp_path):
    """Monte Carlo 기본 동작"""
    propagator = UncertaintyPropagator(artifact_store_path=tmp_path / "artifacts")
...
def test_sensitivity_analysis(tmp_path):
    ...
    # samples_ref → artifact에서 로드
    samples_ref = result["samples_ref"]
    filepath = (tmp_path / "artifacts") / f"{samples_ref}.json"
    assert filepath.exists()
    with open(filepath, "r") as f:
        artifact = json.load(f)
    output_samples = artifact["samples"]
