"""Search Strategy v3 trace verifier unit tests (SSV3-11)."""
...
def test_verify_search_v3_trace_ok(project_root: Path, tmp_path: Path, monkeypatch) -> None:
...
""",
    ...
    issues = verify_search_v3_trace(
        store,
        trace_envelope_artifact_id=str(result.trace_envelope_artifact_id),
        events_artifact_id=str(result.events_artifact_id),
    )
    assert issues == []
...
def test_verify_search_v3_trace_missing_artifacts(project_root: Path, tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CMIS_STORAGE_ROOT", str(tmp_path))
    store = ArtifactStore(project_root=project_root)

    issues = verify_search_v3_trace(store, trace_envelope_artifact_id="ART-missing", events_artifact_id="ART-missing")
    assert "missing_trace_envelope_artifact" in issues or "missing_events_artifact" in issues
