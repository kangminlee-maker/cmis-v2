...
def test_commit_import_run_creates_cub_and_prj(project_root: Path, tmp_path: Path, monkeypatch) -> None:
    ...
    assert rec.focal_actor_context_base_id == "PRJ-test"
    assert rec.focal_actor_context_version == 1
    assert rec.published_focal_actor_context_id == prj_id
...
def test_commit_import_run_is_idempotent(project_root: Path, tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CMIS_STORAGE_ROOT", str(tmp_path))
    ...
    cub_id_1, prj_id_1 = commit_import_run(
        project_root=project_root,
        import_run_id=imp_id,
        policy_mode="reporting_strict",
        focal_actor_context_base_id="PRJ-test",
    ...
    cub_id_2, prj_id_2 = commit_import_run(
        project_root=project_root,
        import_run_id=imp_id,
        policy_mode="reporting_strict",
        focal_actor_context_base_id="PRJ-test",
    ...
    assert cub_id_2 == cub_id_1
    assert prj_id_2 == prj_id_1

    ctx_store = FocalActorContextStore(project_root=project_root)
    try:
        assert ctx_store.list_versions("PRJ-test") == [1]
    finally:
        ctx_store.close()
...
def test_commit_import_run_can_reconcile_after_publish_failure(project_root: Path, tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CMIS_STORAGE_ROOT", str(tmp_path))
    ...
    # simulate publish failure (contexts write)
    import cmis_core.brownfield.outbox as bf_outbox

    orig = bf_outbox._handle_publish_focal_actor_context

    def _boom(*args, **kwargs):
        raise RuntimeError("boom")

    monkeypatch.setattr(bf_outbox, "_handle_publish_focal_actor_context", _boom)
    ...
    assert rec is not None
    assert rec.status == "committed"
    assert rec.published_focal_actor_context_id is None

    # outbox should exist and be failed
    row = conn.execute("SELECT status FROM outbox WHERE idempotency_key = ?", (f"publish_prj:{imp_id}",)).fetchone()
    assert row is not None
    assert str(row[0]) in {"failed", "processing", "pending"}

    # restore handler and reconcile
    monkeypatch.setattr(bf_outbox, "_handle_publish_focal_actor_context", orig)
    rep = reconcile_brownfield_outbox(project_root=project_root, import_run_id=imp_id, retry_failed=True, limit=10)
    assert rep.get("ok") is True

    rec2 = ImportRunStore(conn).get(imp_id)
    assert rec2 is not None
    assert rec2.published_focal_actor_context_id == "PRJ-test-v1"
