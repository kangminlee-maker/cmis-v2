...
class BaseSearchSource(BaseDataSource):
    ...
    # NOTE: web_search로 얻은 결과라도, 아래 도메인/출처는 상대적으로 신뢰도가 높다고 간주합니다.
    # (컨설팅사/증권사/리서치 계열 공개 문서 등)
    ...
    def extract_all_evidence_with_hints(
        ...
        for i, result in enumerate(results):
            ...
            for num in result_numbers:
                ...
                trust = self._source_trust_score(source_url)
                hint_confidence = min(0.85, 0.5 + (0.25 * trust))
                hints.append({
                    "value": num,
                    "context": result.get('title', '')[:100],
                    "snippet": text[:200],
                    "source_url": source_url,
                    "confidence": hint_confidence,  # Hint 기본 신뢰도 + 출처 보너스
                    "source_trust": trust,
                    "result_index": i,
    ...
    @classmethod
    def _normalize_host(cls, url: str) -> str:
        try:
    ...
    @classmethod
    def _source_trust_score(cls, url: str) -> float:
        """web_search 결과의 '출처 신뢰' 점수(0.0~1.0)를 계산합니다.

        목적:
        - web_search로 얻었더라도, 컨설팅사/증권사 등 신뢰도가 상대적으로 높은 문서 출처는
          EvidenceRecord.confidence 및 hints 신뢰도 산정에 반영할 수 있도록 합니다.
        """
        ...
        host = cls._normalize_host(u)
        if host and any(host.endswith(suf) for suf in cls.HIGH_TRUST_DOMAIN_SUFFIXES):
            return 1.0

        # 약한 보너스: 공개 리포트/문서가 pdf로 제공되는 경우가 많음
