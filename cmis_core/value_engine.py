...
class ValueEngine:
    ...
    def evaluate_metrics(
        ...
        Returns:
            (value_records, value_program, metric_evals)
        ...
        로직 (v3):
            ...
            4. MetricEval 생성 (PolicyEngine 검증용)
        """
        results = []
        evidence_bundles = {}
        metric_evals = []
        ...
        for req in metric_requests:
            ...
            if evidence_bundle and evidence_bundle.records:
                ...
                # MetricEval 생성 (evidence-based)
                metric_eval = self._create_metric_eval(
                    record,
            ...
            metric_eval = self._create_metric_eval(record)
            metric_evals.append(metric_eval)
    ...
    # ========================================
    # MetricEval 생성 (PolicyEngine 통합)
    # ========================================

    def _create_metric_eval(
        self,
        value_record: ValueRecord,
        method: Optional[str] = None,
        evidence_bundle: Optional[Any] = None
    ) -> MetricEval:
        """ValueRecord → MetricEval 변환 (PolicyEngine 검증용)

        Args:
            value_record: ValueRecord
            method: 계산 방법 (덮어쓰기 용도)
            evidence_bundle: EvidenceBundle (있으면 사용)

        Returns:
            MetricEval
        """
        quality = value_record.quality or {}

        # literal_ratio (Evidence 직접 사용 비율)
        literal_ratio = quality.get("literal_ratio", 0.0)

        # spread_ratio (불확실성 비율)
        spread_ratio = quality.get("spread_ratio")

        # confidence
        confidence = quality.get("confidence", 0.0)

        # methods_used
        method_used = method or quality.get("method", "unknown")
        methods_used = [method_used]

        # prior_ratio (Prior 기여 비율)
        prior_ratio = quality.get("prior_ratio", 0.0)

        # prior_types_used
        prior_types_used = []
        if prior_ratio > 0:
            prior_type = quality.get("prior_type", "unknown")
            prior_types_used = [prior_type] if prior_type != "unknown" else []

        # convergence_ratio (다중 방법 간 불일치 비율)
        convergence_ratio = quality.get("convergence_ratio")
        ...
        return MetricEval(
            ...
            prior_types_used=prior_types_used,
