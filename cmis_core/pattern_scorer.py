...
class PatternScorer:
    ...
    def calculate_execution_fit(
        ...
        capability_score = self._calculate_capability_match(
            ...
            project_context.assets_profile.get("capability_traits", [])
        )
        
        # 2. Constraint 체크 (0.3 가중치)
        constraint_score = self._calculate_constraint_satisfaction(
            pattern.constraint_checks,
            project_context.constraints_profile.get("hard_constraints", [])
        )
    ...
    def _load_project_context(self, project_context_id: str) -> Any:
        """Project Context 로딩
        
        Phase 2: 메모리/딕셔너리에서 로딩
        Phase 3: Project Context Store에서 로딩
        """
        # Phase 2: 테스트용 간단한 로딩
        # 실제로는 Store에서 로딩해야 함
