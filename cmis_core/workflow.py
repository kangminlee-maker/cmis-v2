...
class WorkflowOrchestrator:
    ...
    def _load_canonical_workflows(self) -> Dict[str, Any]:
        ...
        """
        cmis_yaml_path = self.project_root / "cmis.yaml"
        
        if not cmis_yaml_path.exists():
            return {}
        ...
        try:
            ...
            workflows_list = data.get("cmis", {}).get("canonical_workflows", [])
            
            # list → dict (id → workflow)
            workflows = {w["id"]: w for w in workflows_list}
    ...
    def run_workflow(
        ...
        if policy_mode is None:
            policy_mode = self.role_policy_map.get(role_id, "reporting_strict")
