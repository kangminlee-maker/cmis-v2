...
class TaskRouter:
    ...
    def _init_from_config(self):
        """Config에서 Route 로드"""
        # cmis.yaml의 llm 섹션 (없으면 빈 dict)
        llm_config = {}
        
        if hasattr(self.config, 'cmis'):
        ...
        for task_str, route_config in routes_config.items():
            try:
                ...
                route = TaskRoute(
                    task_type=task_type,
                    provider_id=route_config.get("provider", "openai"),
                    model_name=route_config.get("model", "gpt-4o-mini"),
...
class LLMRegistry:
    ...
    def _init_from_config(self):
        ...
        # cmis.yaml llm.providers (없으면 빈 dict)
        llm_config = {}
        ...
        default_mode = llm_config.get("default_mode", "mock")
        
        if default_mode in self._providers:
            self._providers["__default__"] = self._providers[default_mode]
            logger.info(f"Default provider set: {default_mode}")
        else:
            # Fallback: MockLLM
            logger.warning(f"Default provider '{default_mode}' not found, using mock")
            from .providers import MockLLM
            self._providers["__default__"] = MockLLM()
...
class LLMService:
    ...
    def call(
        ...
        **kwargs
        ...
        logger.info(
            f"LLM call: {task_type.value} → {route.provider_id}/{route.model_name} "
...
def create_llm_service(
    ...
    # Default route 등록 (Config에 없을 때)
    if not service.router.routes:
        # Default provider 결정
        default_provider = service.registry.get_provider("__default__")
        
        if default_provider:
            provider_info = default_provider.get_info()
            provider_id = provider_info.get("provider_id", "mock")
            model_name = provider_info.get("model", "mock")
        else:
            provider_id = "mock"
            model_name = "mock"
        ...
        service.router.register_route(default_route)
        logger.info(f"Default route created: {provider_id}/{model_name}")
